...
def cacheable(expires=None, public=False):
    ...
    are set to expire in now + expires seconds.

    If public is set to True, cache-control header will allow intermediary
    caches to cache the response. If cookie based sessions are being used this
    should only ever be set to True when we know Set-Cookie header will be
    stripped downstream.
